...
# It relies on trained ANN for the location, developed using WindStation results and the programa WTM

def modify_epw_file(filename):
            ...
            if len(parts) >= 35 and parts[0].isdigit():
                try:
                    ...
                    result = subprocess.run(
                        ["python", "<ANNFILE>", str(wind_speed), str(wind_dir), str(station_x), str(station_y)],
                        capture_output=True,
                        text=True
                    )
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python <ANNFILE> <epw_filename>")
    else:
        modify_epw_file(sys.argv[1])
